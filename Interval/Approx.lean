...
attribute [approx] mem_approx_smul

@[approx] lemma subset_approx_neg [Neg R] [Neg A] [Approx A R] [ApproxNeg A R] {a : Set R} (x : A)
    (ax : a ⊆ approx x) : -a ⊆ approx (-x) := by
  refine subset_trans ?_ (approx_neg x)
  intro b m; exact ax m

@[approx] lemma subset_approx_add [Add R] [Add A] [Approx A R] [ApproxAdd A R] {a b : Set R}
    {x y : A} (ax : a ⊆ approx x) (yb : b ⊆ approx y) : a + b ⊆ approx (x + y) :=
  subset_trans (add_subset_add ax yb) (approx_add x y)

@[approx] lemma subset_approx_sub [Sub R] [Sub A] [Approx A R] [ApproxSub A R] {a b : Set R}
    {x y : A} (ax : a ⊆ approx x) (yb : b ⊆ approx y) : a - b ⊆ approx (x - y) :=
  subset_trans (sub_subset_sub ax yb) (approx_sub x y)

@[approx] lemma subset_approx_mul [Mul R] [Mul A] [Approx A R] [ApproxMul A R] {a b : Set R}
    {x y : A} (ax : a ⊆ approx x) (yb : b ⊆ approx y) : a * b ⊆ approx (x * y) :=
  subset_trans (mul_subset_mul ax yb) (approx_mul x y)

@[approx] lemma subset_approx_inv [InvolutiveInv R] [Inv A] [Approx A R] [ApproxInv A R] {s : Set R}
    {x : A} (sx : s ⊆ approx x) : s⁻¹ ⊆ approx x⁻¹ :=
  subset_trans (inv_subset_inv.mpr sx) (approx_inv x)

@[approx] lemma subset_approx_star [Star R] [Star A] [Approx A R] [ApproxStar A R] {s : Set R}
    {x : A} (sx : s ⊆ approx x) : star '' s ⊆ approx (star x) :=
  subset_trans (image_mono sx) (approx_star x)

@[approx] lemma subset_approx_div [Div R] [Div A] [Approx A R] [ApproxDiv A R] {a b : Set R}
    {x y : A} (ax : a ⊆ approx x) (yb : b ⊆ approx y) : a / b ⊆ approx (x / y) :=
  subset_trans (div_subset_div ax yb) (approx_div x y)

@[approx] lemma mem_approx_neg [InvolutiveNeg R] [Neg A] [Approx A R] [ApproxNeg A R] {a : R}
    {x : A} (ax : a ∈ approx x) : -a ∈ approx (-x) := by
  apply approx_neg x; simpa only [mem_neg, neg_neg]

@[approx] lemma mem_approx_add [Add R] [Add A] [Approx A R] [ApproxAdd A R] {a b : R} {x y : A}
...
@[approx] lemma mem_approx_sub [Sub R] [Sub A] [Approx A R] [ApproxSub A R] {a b : R} {x y : A}
    (ax : a ∈ approx x) (yb : b ∈ approx y) : a - b ∈ approx (x - y) := by
...
@[approx] lemma mem_approx_mul [Mul R] [Mul A] [Approx A R] [ApproxMul A R] {a b : R} {x y : A}
    (ax : a ∈ approx x) (yb : b ∈ approx y) : a * b ∈ approx (x * y) := by
...
@[approx] lemma mem_approx_inv [InvolutiveInv R] [Inv A] [Approx A R] [ApproxInv A R] {a : R}
    {x : A} (ax : a ∈ approx x) : a⁻¹ ∈ approx x⁻¹ := by
  apply subset_approx_inv (singleton_subset_iff.mpr ax)
  simp only [inv_singleton, mem_singleton_iff]

@[approx] lemma mem_approx_star [Star R] [Star A] [Approx A R] [ApproxStar A R] {a : R} {x : A}
...
@[approx] lemma mem_approx_div [Div R] [Div A] [Approx A R] [ApproxDiv A R] {a b : R} {x y : A}
    (ax : a ∈ approx x) (yb : b ∈ approx y) : a / b ∈ approx (x / y) := by
...
/-- Test `approx` for `⊆ approx`-/
example [Field R] [ApproxField A R] (x y z : A) :
    approx x + approx y * -approx z ⊆ approx (x + y * -z) := by approx

/-- Test `approx` for `∈ approx`-/
example [Field R] [ApproxField A R] (a b c : R) (x y z : A)
    (am : a ∈ approx x) (bm : b ∈ approx y) (cm : c ∈ approx z) :
    a + b * -c ∈ approx (x + y * -z) := by approx
...
@[approx] lemma Icc_subset_approx [Approx A R] [Preorder R] [ApproxConnected A R] {a b : R} {x : A}
