...
variable {x : Floating} {a : ℝ}

lemma valid_neg : Valid (-x.n) x.s where
...
@[simp] lemma n_neg : (-x).n = -x.n := rfl
@[simp] lemma s_neg : (-x).s = x.s := rfl
...
@[simp] lemma neg_eq_nan_iff : -x = nan ↔ x = nan := by
