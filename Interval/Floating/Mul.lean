...
lemma mul_norm_correct (n : UInt128) (up : Bool) (n0 : n ≠ 0) (lo : n.toNat ≤ mul_n_max) :
    (mul_norm n up).1.toNat ∈ Ico (2^62) (2^63) ∧
      Rounds (((mul_norm n up).1.toNat : ℝ) * 2 ^ (64 - ((mul_norm n up).2.toNat : ℤ)))
        (n.toNat : ℝ) up := by
  rw [mul_norm]
  ...
  · simp only [r_eq, cond_true] at hp
    ...
    · simp only [beq_iff_eq] at r_eq
      have not_up : up = true := by
        ...
        omega
      simp only [not_up, Bool.and_true, up62, Nat.cast_pow, Nat.cast_ofNat] at hb ⊢
      clear not_up up
      ...
      simp only [UInt64.toNat_sub'' s1', UInt64.toNat_one, Nat.cast_sub s1, Nat.cast_one,
        pow_mul_zpow t0, Nat.cast_ofNat]
...
lemma mul_finish_correct (n : UInt64) (s : Int128) (up : Bool)
    ...
    Rounds (mul_finish n s up norm) ((n.toNat : ℝ) * 2^((s : ℤ) - 2^63)) up := by
  rw [mul_finish]
  simp only [bif_eq_if, beq_iff_eq]
  have nn : 0 ≤ n.toInt64 := by
    simp only [← not_lt, Int64.isNeg_eq_le, norm.2, not_not, UInt64.toUInt64_toInt64]
  by_cases h0 : s.n.hi = 0
  · simp only [h0, ↓reduceIte, rounds_iff, ne_eq]
    intro _
    rw [val]
  ...
  · simp only [h0, ite_false]
    ...
    · simp only [sn, ite_true]
      ...
      · simp only [Bool.false_eq_true, ↓reduceIte, Int.reducePow, rounds_iff, ne_eq, zero_ne_nan,
          ...
          imp_self]
      · simp only [ite_true, ne_eq, min_norm_ne_nan, not_false_eq_true, val_min_norm, rounds_iff,
          true_imp_iff]
        ...
        norm_num
        omega
    · simp only [Int128.isNeg_iff, decide_eq_true_eq, not_lt] at sn
      simp only [Int128.isNeg_iff, decide_eq_true_eq, Int.reducePow, rounds_iff, ne_eq,
        ite_eq_right_iff, isEmpty_Prop, not_lt, sn, IsEmpty.forall_iff, not_true_eq_false]
...
lemma approx_mul_of_nonneg {x y : Floating} {up : Bool} {x0 : 0 ≤ x.val} {y0 : 0 ≤ y.val}
    (ax : approx x x') (ay : approx y y') :
    Rounds (mul_of_nonneg x y up x0 y0) (x' * y') up := by
  have t0 : (2 : ℝ) ≠ 0 := by norm_num
  ...
  by_cases xn : x = nan; simp only [xn, not_nan_nonneg] at x0
  by_cases yn : y = nan; simp only [yn, not_nan_nonneg] at y0
  ...
  simp only [hz, hm, he, ← ax, ← ay] at ce n ⊢
  ...
  · simp only [z0, dite_false] at n ⊢
    ...
    simp only [hm, mem_Ico, rounds_same] at cn
    have cf := mul_finish_correct m.1 e up cn.1
    simp only [rounds_iff, n, ne_eq, not_false_iff, true_imp_iff] at cf
    replace cn := cn.2
    induction up
    · simp only [Bool.false_eq_true, ↓reduceIte, UInt64.natCast_toNat,
        ← le_div_iff₀ (G₀ := ℝ) two_zpow_pos] at cn cf ⊢
      ...
      simp only [← le_div_iff₀ (G₀ := ℝ) two_zpow_pos, div_le_iff₀ (G₀ := ℝ) two_zpow_pos,
        mul_assoc, mul_div_assoc, ←zpow_sub₀ t0, ←zpow_add₀ t0, ce, UInt64.toInt]
...
lemma approx_mul (x y : Floating) (up : Bool) (ax : approx x x') (ay : approx y y') :
    Rounds (x.mul y up) (x' * y') up := by
  rw [mul]
  ...
  simp only [approx, xn, false_or, yn] at ax ay
  simp only [xn, yn, or_self, bif_eq_if, isNeg_iff, hz, dite_false, Int64.isNeg, ← ax, ← ay]
  by_cases ze : z = true
  · simp only [ze, bne_iff_ne, ne_eq, decide_eq_decide, Bool.xor_true, ite_true] at hz ⊢
    apply rounds_neg
...
lemma mul_le {x y : Floating} (n : x.mul y false ≠ nan) :
  ...
  simpa [rounds_iff, n] using approx_mul x y false approx_val approx_val

/-- `mul _ _ true` rounds up -/
lemma le_mul {x y : Floating} (n : x.mul y true ≠ nan) :
    x.val * y.val ≤ (x.mul y true).val := by
  simpa [rounds_iff, n] using approx_mul x y true approx_val approx_val
