...
@[simp] lemma low_d_le (g s : UInt64) : (lower g s).2.toNat ≤ g.toNat := by
  ...
  split_ifs with h
  · simp only [UInt64.sub_zero, UInt64.toNat_le_toNat, h.le]
  · apply le_of_eq
    simp only [UInt64.sub_sub_cancel]
...
@[simp] lemma log2_g_le_62 {n : Int64} (nm : n ≠ .minValue) : n.uabs.log2 ≤ 62 := by
  by_cases n0 : n = 0
  · simp only [n0, Int64.uabs_zero, UInt64.log2_zero, UInt64.nonneg]
  rw [UInt64.le_iff_toNat_le, UInt64.toNat_log2, u62]
  suffices h : n.uabs.toNat.log2 < 63 by omega
  refine (Nat.log2_lt ?_).mpr ?_
  · simpa only [← UInt64.ne_zero_iff_toNat_ne_zero, Int64.uabs_ne_zero_iff]
  · rwa [Int64.toNat_uabs, Int64.natAbs_lt_pow_iff]

@[simp] lemma log2_g_le_62' {n : Int64} (nm : n ≠ .minValue) : n.uabs.toNat.log2 ≤ 62 := by
  rw [←u62, ← UInt64.toNat_log2, ←UInt64.le_iff_toNat_le]
  exact log2_g_le_62 nm

/-- `lower` returns shifts under `62` -/
@[simp] lemma low_d_le_62 {n : Int64} (nm : n ≠ .minValue) (s : UInt64) :
    (lower (62 - n.uabs.log2) s).2.toNat ≤ 62 := by
  refine le_trans (low_d_le _ s) ?_
  rw [UInt64.toNat_sub'' (log2_g_le_62 nm), u62]
  apply Nat.sub_le

/-- `low` doesn't overflow -/
@[simp] lemma low_lt {n : Int64} (nm : n ≠ .minValue) (s : UInt64) :
    n.uabs.toNat * 2 ^ (lower (62 - n.uabs.log2) s).2.toNat < 2 ^ 63 := by
  refine lt_of_lt_of_le (Nat.mul_lt_mul_of_lt_of_le (Nat.lt_log2_self (n := n.uabs.toNat))
    (Nat.pow_le_pow_right (by positivity) (low_d_le _ s)) (by positivity)) ?_
...
/-- `low` doesn't overflow -/
@[simp] lemma low_lt' {n : Int64} (nm : n ≠ .minValue) (s : UInt64) :
    |(n : ℤ) * 2 ^ (lower (62 - n.uabs.log2) s).2.toNat| < 2 ^ 63 := by
  have h := low_lt nm s
  generalize (lower n.toUInt64 s).2.toNat = k at h
  refine lt_of_le_of_lt (abs_mul _ _).le ?_
  simp only [←Int.natCast_natAbs, ← Int64.toNat_uabs, abs_pow, abs_two]
  rw [←Nat.cast_lt (α := ℤ)] at h
  simpa only [Nat.cast_mul, Nat.cast_pow, Nat.cast_ofNat] using h

/-- `lower` respects `ℤ` conversion -/
lemma coe_low_s {n : Int64} {s : UInt64} (nm : n ≠ .minValue) :
    ((n <<< (lower (62 - n.uabs.log2) s).2 : Int64) : ℤ) =
      (n : ℤ) * 2^(lower (62 - n.uabs.log2) s).2.toNat := by
...
lemma of_ns_ne_nan {n : Int64} {s : UInt64} (nm : n ≠ .minValue) :
    n <<< (lower (62 - n.uabs.log2) s).2 ≠ .minValue := by
...
lemma of_ns_norm {n : Int64} {s : UInt64} (n0 : n ≠ 0) (nm : n ≠ .minValue) :
    n <<< (lower (62 - n.uabs.fast_log2) s).2 ≠ 0 →
      n <<< (lower (62 - n.uabs.fast_log2) s).2 ≠ Int64.minValue →
      (lower (62 - n.uabs.fast_log2) s).1 ≠ 0 →
      2 ^ 62 ≤ (n <<< (lower (62 - n.uabs.fast_log2) s).2).uabs := by
  intro _ _ sm
  simp only [UInt64.fast_log2_eq] at sm ⊢
  rw [UInt64.le_iff_toNat_le, up62, Int64.toNat_uabs, coe_low_s nm, Int.natAbs_mul, Int.natAbs_pow,
    ua2, ← Int64.toNat_uabs]
  rw [lower] at sm ⊢
...
/-- `of_ns` is valid -/
lemma valid_of_ns {n : Int64} {s : UInt64} (nm : n ≠ .minValue) (n0 : n ≠ 0) :
    let t := lower (62 - n.uabs.fast_log2) s
    Valid (n <<< t.2) t.1 := {
  zero_same := by
...
@[irreducible] def of_ns (n : Int64) (s : UInt64) : Floating :=
  if nm : n = .minValue then nan else
...
@[simp] lemma of_ns_nan (s : UInt64) : of_ns .minValue s = nan := by
...
/-- `of_ns` preserves `val` -/
lemma val_of_ns {n : Int64} {s : UInt64} (nm : n ≠ .minValue) :
    (of_ns n s).val = (n : ℤ) * 2^(s.toInt - 2^63) := by
  rw [of_ns, val]
  simp only [nm, dite_false]
  by_cases n0 : n = 0
  · simp only [n0, dite_true, n_zero, Int64.coe_zero, Int.cast_zero, s_zero, zero_mul]
  ...
  simp only [low_s_2_eq, mul_assoc, ne_eq, OfNat.ofNat_ne_zero, not_false_eq_true, pow_mul_zpow,
    mul_eq_mul_left_iff, Int.cast_eq_zero, Int64.coe_eq_zero, n0, or_false, Nat.cast_sub low_le_s',
    UInt64.toInt]
  ring_nf

/-- `of_ns` doesn't create `nan` -/
@[simp] lemma of_ns_eq_nan_iff {n : Int64} {s : UInt64} : of_ns n s = nan ↔ n = .minValue := by
  constructor
  · intro nm; contrapose nm
    rw [of_ns, ext_iff]
    by_cases n0 : n = 0
    · simp only [n0, Int64.zero_ne_min, dite_true, dite_eq_ite, ite_false, n_zero, n_nan, s_zero,
      s_nan, false_and, not_false_eq_true]
    · simp only [nm, n0, dite_false, n_nan, of_ns_ne_nan nm, s_nan, false_and, not_false_eq_true,
        UInt64.fast_log2_eq]
  · intro h; rw [h, of_ns_nan]

/-- `of_ns` doesn't create `nan` -/
@[simp] lemma of_ns_ne_nan_iff {n : Int64} {s : UInt64} : of_ns n s ≠ nan ↔ n ≠ .minValue := by
