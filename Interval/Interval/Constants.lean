...
def n : ℕ := 70
def x : ℚ := 1/2
def radius : ℚ := |x| ^ (n + 1) / (1 - |x|)
def mid : ℚ := -∑ i ∈ Finset.range n, x ^ (i + 1) / (i + 1 : ℚ)
...
lemma close : |Real.log (1/2) - mid| ≤ radius := by
  ...
  simp only [mid, x, radius, Rat.cast_neg, Rat.cast_sum, Rat.cast_div, Rat.cast_pow, Rat.cast_add,
    Rat.cast_natCast, Rat.cast_one, neg_neg, Rat.cast_ofNat, Rat.cast_abs, Rat.cast_sub]
...
lemma inv_close : |(Real.log 2)⁻¹ - inv_mid| ≤ inv_radius := by
  ...
  have mid0 : 0 < |(mid : ℝ)| := by simp only [← Rat.cast_abs, Rat.cast_pos]; native_decide
...
@[irreducible] def log_half : Interval := Log2.mid
...
@[approx] lemma approx_log_half : Real.log (1/2) ∈ approx log_half := by
  rw [(by native_decide : log_half = Log2.interval)]
